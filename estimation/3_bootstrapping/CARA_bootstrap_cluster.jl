...
const USE_KNITRO=true

### Note this code is written for the GSB yens ##
ENV["ARTELYS_LICENSE_NETWORK_ADDR"] = "license4.stanford.edu"

today = Dates.today()

# check command line argument (should be the job array index: 1, 2, ..., 438)
# note that command line arguments are stored as array of strings so need to cast to int
array_index = parse(Int64, get(ENV, "SLURM_ARRAY_TASK_ID", "0"))
...
out_dir = bootstrap_out_dir * "bs_" * "$array_index" * "/"
