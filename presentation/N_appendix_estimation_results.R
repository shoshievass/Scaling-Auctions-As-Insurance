...
param_names <- bootstrap_sumstats %>%
  select(parameter) %>%
  mutate(
...
gmm_bid_fits_matched <-
  ...
  left_join(
    minimal_match_df,
    by = c("contract_no" = "contract_no", "bidder_id" = "bidder_id_sequential", "item_id" = "item_id_sequential")
  ) %>%
  mutate(
    bid_fit = bid_fit * w,
...
compare %>%
  ...
  ggplot(aes(x = bidder_score, y = winning_score)) +
  geom_point() +
